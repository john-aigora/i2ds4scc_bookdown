...
# Brief Introduction of Machine Learning {#machine-learning}
...
trees' predictions. This way ... <!-- Finish this part -->
...
<!-- Vanessa: Maybe add some little section before each part of the code -->
...
We can use `autoplot()` function to take a quick look at the `tuning` object.<!-- tian: explain the graph here? --> 

Next, we will obtain a reliable estimate of the quality of the model. We first select the best combination of parameters using `select_best()` function. The quality of the model can be estimated with a number of metrics. Here we decided to use roc_auc (Area Under the Receiver Operating Characteristic Curve) <!-- tian: I'm confused here. we are choosing the best paras, but where can we see the quality of the model? -->
