...
The data can be scaled or not before applying PCA. As a general recommendation, scaling should be applied if the variables are in different units or scales, as in the case of the examples being used. In case the variables are all in the same unit/scale, as in a situation where there are only sensory variables using a 10 cm scale, PCA without scaling would be more appropriate. Just as illustration, we will be running PCA without and with scaling with the wine set example.
