...
<!-- Change order: First Supervised and then Unsupervised? (Vanessa) -->
...
R contains fantastic systems for building machine learning models. The tidymodels framework (https://www.tidymodels.org/) is a collection of packages for modeling and machine learning using tidyverse principles, and among one of the most popular framework in R for machine learning. 
...
<!-- tian: do we need to explain cross validated by product and by consumer here? -->
...
<!-- tian: a diagram could help here: https://www.tidymodels.org/start/resampling/#resampling -->
...
<!-- tian: maybe we need to explain stratify here more, does it mean that in each of those 5 fold, the ratio of wine_type will be similar to the original wine_type? -->
...
<!-- tian: why we choose random forest here? -->
...
<!-- there is a warning message about grid_regular, it's deprecated in tune now. -->
<!-- for me is working fine (Vanessa) -->
...
<!-- tian: I'm confused here. we are choosing the best paras, but where can we see the quality of the model? -->
...
A very important part in building machine learning models is to assess the quality of the models. To do so, first we can use the model to predict on the testing data set (`wind_testing`), which the model has not seen yet. 
...
A very pleasant method for visualize the classification results is to use a confusion matrix. Confusion matrix is a table where each row represents instances in the actual class, while each column represents the instances in a predicted class. Using the `autoplot()` function, We can see that in our wine type prediction, it's a 100% match!

<!--and the associated statistics (accuracy, sensitivity specificity, precision, F1 score and a lot more: https://en.wikipedia.org/wiki/Confusion_matrix). -->
...
<!-- tian: Do we want to include another regression example? it's more common in the sensory consumer science context -->
