...
A schematic of a data scientific workflow is shown in Figure \@ref(fig:ds-workflow) (Diagram inspired by @Wickham2016).  Each section is described in greater detail below.

```{r ds-workflow, echo=FALSE, eval=TRUE, fig.cap='Data scientific workflow.', out.width='90%'}
knitr::include_graphics("images/data_science_workflow2.jpg")
```
...
The same also applies for the analysis part. Documenting which analyses and which parameters were used ensure reproducible research. If at first, documenting you code may seem *a loss of time*, it will pay back later when you will access your code again later in time, as decisions taken while you are writing your code may not be so clear anymore afterwards.
