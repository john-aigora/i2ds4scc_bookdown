...
<!-- Maybe "differences between TWO products" could be written as "differences between products" -->
<!-- As the broad categories are being described here, maybe it would be clearer if the sentence of the third bullet point started with Affective Testing and then focused on the hedonic test-->
...
<!-- Although typically the difference test results consists in a series of binary series, it is a bit confusing to me since this is not the case of common tests like DFC---> 
Each test category generates its own type of data and related statistical questions in relation to the objectives of the study. Typically, data from difference tests consist in series of binary answers (correct/failed) depending on whether judges successfully picked the odd sample(s) among a set of three or more samples. These data are used to determine whether the number of correct choices is above the level expected by chance.
