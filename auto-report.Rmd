...
var_levels <- read_xlsx(file_path, sheet="Levels") %>% 
  inner_join(var_names, by=c("Question"="Code")) %>% 
  dplyr::select(Levels, Code, Name)

demographic <- read_xlsx(file_path, sheet="Data", col_names=var_names$Name, skip=1) %>% 
  dplyr::select(Judge, `Living area`, Housing, `Marital status`, `Income range`, `Occupation`) %>% 
  pivot_longer(-Judge, names_to="Name", values_to="Response") %>% 
  left_join(var_levels, by=c("Name", "Response"="Code")) %>% 
  dplyr::select(-Response) %>% 
  pivot_wider(names_from=Name, values_from=Levels) %>% 
  dplyr::select(-Judge)
