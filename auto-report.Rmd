...
Note that the spreadsheet is complete, we create the file using `saveWorkbook()` by specifying the name of the workbook `wb` and its path through `file`. In case such workbook already exists, it can be overwritten using `overwrite`. 
...
We start with pre-defining our styling parameters that we then apply to our different tables or slides. This is not a rule, yet we strongly recommend you doing so: If you start with creating your different styling parameters at the start of your script file, these lines of code do not interfere with the rest of your code. And keeping well-structured code gains in clarity, and will be game changer when trouble-shooting for some errors.

In fact, we would recommend you going one step further and store all this styling parameters in a separate file that you can then load any time you need it using `source()`. By doing so, you **reduce** the size of your script file (hence gaining in clarity), while **harmonizing** all your exports by **centralizing** your code in one unique place. The last point is particularly important since any change/update done in one place will then be automatically applied to any new report.
