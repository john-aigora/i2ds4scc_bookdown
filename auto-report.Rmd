...
```{r 1.1 creating a PPT deck, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
library(officer)
pptx_obj <- read_pptx() 
```
...
#### Positioning Information on the Slide
...
```{r 4.1 basic code, eval=TRUE, echo=TRUE}
ft_data <- senso_mean %>%
...
```{r 4.5 demonstration cont.1, eval=TRUE, echo=TRUE}
ft_table <- ft_table %>% 
  fontsize(size = 11) %>%
...
```{r 5.1 rvg package, eval=TRUE, echo=TRUE}
...
```{r 5.2 rvg example, eval=TRUE, echo=TRUE}
library(rvg)
pptx_obj <- pptx_obj %>%
  add_slide(layout = "Title and Content", master = master) %>%
...
```{r 5.3 mschart example, eval=TRUE, echo=TRUE}
library(mschart)
