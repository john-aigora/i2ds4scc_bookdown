...
```{r, echo=FALSE, eval=TRUE}
sensochoc_tb <- sensochoc[1:5,] %>% 
  as_tibble()
```

>Due to the reduction of the output, here you'll notice that the information regarding the tibble is different in the book and on your screen. Since we limit ourselves to the 5 first rows here, R prints that `sensochoc_tb` is a `tibble: 5 x 18` while on your screen, it should be defined as `tibble: 348 x 18`. This is an inconsistency that will occur through the book as we will reduce the size of some of the outputs. If such thing should occur, do not worry as the error probably does not come from you! 
...
>It is also worth reminding the readers that we deliberately **do not print** each and every output generated. Instead, we advise the reader to re-write the code and run it in parallel as they read the book. The outputs will then be printed on their screen. Also, in some cases, we took the liberty to reduce the outputs (e.g. only showing the first rows and/or columns) which can lead to some inconsistencies between what is printed in the book, and what is shown on your screen (e.g. the size of a tibble). So please be aware that this can (and will) happen, and when that happen, trust the results on your screen!  
