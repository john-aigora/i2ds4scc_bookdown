...
How many did you count? Probably this was a tedious task and it took you a few seconds because it involved **attentive processing**. The list of numbers did not have any hint or called preattentive attributes that could help you to easily distinguish the number five from the other numbers, so we are forced to perform a sequential search throughout the whole list.
...
In a situation where we are interested to display the mean for the appearance attributes evaluated in the 11 biscuits, we would need first to prepare the data frame, which means, selecting the wanted attributes and calculating the mean scored by panelists.
...
```{r flextable_1, fig.cap='Basic Flextable', fig.align='center', echo=FALSE, out.width="85%", eval=TRUE}

knitr::include_graphics("images/flextable_1.jpeg")
...
After the basic aesthetic changes in the table, we can add additional elements such as the table title and also, depending on our purpose and where we want to call the audience's attention, use other strategies. Let's here highlight the optimized formulation.
...
```{r flextable_2, fig.cap='Basic Flextable', fig.align='center', echo=FALSE, out.width="85%", eval=TRUE}

knitr::include_graphics("images/flextable_2.jpeg")

```

The table now looks much nicer and can be further improved with other preattentive attributes according to the main objective of your message. 
...
Focusing on the consumer study (Consumer Test.xlsx), suppose we want to display a table with the average the consumers spent in the booth and the number of biscuits eaten for each product. We need to transform the time columns (expressed as min and s) to a double format and express them as minutes units, and then group them by product and get the average for the time spent and the number of biscuits eaten.
...
```{r gt_table, fig.cap='Consumption time and number of biscuits eaten', fig.align='center', echo=FALSE, out.width="85%", eval=TRUE}
...
The `{gt}` package offers several resources to make beautiful tables using preattentive attributes. As an example, let's now focus on the average number of biscuits eaten only, since we saw that the consumption time was very similar among products. The idea is to use pre-attentive attributes for the audience to clearly see the formulation ta got the most to the least biscuits eaten. Let's first prepare the data and calculate the overall time consumption considering all products.
...
biscuits_palette <- col_numeric(c("#FEF0D9", "#990000"), domain = c(min_nb_biscuits, max_nb_biscuits), alpha = 0.75)

consumer_table_final <- consumer_gt_table_2 %>% 
    data_color(columns = c(`Nb biscuits`),
               colors = biscuits_palette)

```

```{r gt_table_2, fig.cap='Number of biscuits eaten', fig.align='center', echo=FALSE, out.width="85%", eval=TRUE}
