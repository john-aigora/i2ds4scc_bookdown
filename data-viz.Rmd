...
Let's now focus on how to make nice plots and tables in R.
...
Let's starting using the package `{flextable}` to build tables using as example the sensory data (Sensory Profile.xlsx). Let's first load the necessary libraries.
...
In a situation where we are interested to display the mean for the appearance attributes evaluated in the 11 biscuits, we would need first to prepare the data frame, which means, select the wanted attributes and calculate the mean scored by panelists.
...
As we can see, we have a very simple and overcrowded table that clearly needs some designing work to make it nicer.
We can start improving wit with some basic elements such as standardizing the number of decimals, changing the font size, adjusting the cell widths and heights, highlighting the header and aligning the text.
...
After the basics aesthetics changes in the table, we can add additional elements such as table title and also, depending on our purpose and where we want to call the audience attention, use other strategies. Let's here highlight the optimized formulation.
...
Focusing on the consumer study (Consumer Test.xlsx), suppose we want to display a table with the average the consumer spent in the booth and the number of biscuits eaten for each product. We need to transform the time columns (expressed as min and s) to a double format and express as minutes unit, and then group by product and get the average for the time spent and number of biscuits eaten.
...
# Consumption Time and Number of Biscuits
...
consumer_gt_tablel <- mean_consumer %>%
  gt () %>%
...
Note that we used Markdown to style the title and subtitle by wrapping the values passed to title or subtitle with the md() function. For Markdown ** makes the text bold and * makes the test italic.

The `{gt}` package offers several resources to make beautiful tables using preattentive attributes. As an example, lets a

<!--  packages gt, gtextras, kable, kableExtra -->
