...
There are any clustering approaches and algorithms. They can be categorized into a different types including *exclusive* (e.g. k-means), *hierarchical* (see Section \@ref(hac) for an example) and *probabilistic* (e.g. Gaussian Mixture Model). The first two are most widely used and well known in the sensory field. 
...
For instance, with workflows, it is easy to try different ML algorithms by simply creating multiple models.
