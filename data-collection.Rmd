...
```{r}
library(SensoMineR)
wdes5P10J <- WilliamsDesign(5)
```

Suppose you want to include 20 judges in the experiment, you would then need to duplicate the initial design.

```{r}
wdes5P20J <- do.call(rbind, replicate(2, wdes5P10J, simplify=FALSE))
rownames(wdes5P20J) <- c(paste("judge", 1:20, sep=""))
```
...
As the total number of judges in your study may not always be exactly known in advance (participants not showing up to your test, extra participants recruited at the last minute), it can be useful to add some flexibility to the design. Of course, additional rows would depart from the perfectly balanced design, but it is possible to optimize them using Federov's algorithm thanks to the `optFederov()` function of the `{AlgDesign}` package, by specifying `augment = TRUE`. For example we can add three more judges to the Williams Latin square design that we just built for 5 products and 10 judges, hence leading to a total number of 13 judges.
...
wdes5P10J <- WilliamsDesign(nbP)
tab <- cbind(prod=as.vector(t(wdes5P10J)),judge=rep(1:nbJ,each=nbR),rank=rep(1:nbR,nbJ))

optdes5P13J <- optFederov(~prod+judge+rank,data=tab,augment=TRUE,nTrials=nbJ*nbP,rows=1:(nbJ*nbP),nRepeats = 100)
xtabs(optdes5P13J$design)
```
...
optdes <- optimaldesign(nbP,nbP,nbR)$design
tab <- cbind(prod=as.vector(t(optDes)),judge=rep(1:nbJ,each=nbR),rank=rep(1:nbR,nbJ))
add <- cbind(prod=rep(1:nbP,nbR),judge=rep(nbJ+1,nbP*nbR),rank=rep(1:nbR,each=nbP))

optdes5P14J <- optFederov(~prod+judge+rank,data=rbind(tab,add),augment=TRUE,nTrials=(nbJ+1)*nbP,rows=1:(nbJ*nbP),nRepeats = 100)
```
...
The `optimaldesign()` function of `{SensoMineR}` can be used to search for a balanced incomplete block design.
...
You can also use the `{crossdes}` package to generate an BIBD with this simple syntax: `find.BIB(trt, b, k, iter)`, with trt = number of products, b = number of judges, k = number of samples per judge, iter = number of iteration. With this package, you can apply `isGYD()` to know if an incomplete design is balanced or not. If the design is a BIBD, you may then use `williams.BIB()` to combine it with a Williams design to balance carryover effects.
...
#### Mixture designs
In many product development projects (e.g. in the food industry, in the personal care industry), developers work on products' formula. It means that factors are ingredients and that their proportions are interdependent (the total sum of all components of a mixture must be 100%). Therefore, these factors must be treated as mixture components.

Which package can be used to design mixture DoE

Common representation on ternary diagrams. Easy with the `{Ternary}` package.


#### Screening designs
Factorial and mixture designs are belong to the product developers' essential toolkit. In practice however, they can only include a relatively limited number of factors. For this reason most influential factors of interest must be identified beforehand. Of course, product development is not a monolithic process and in the early stages of project it could be extremely useful to use a screening design in combination to sensory evaluation.
Plackett-Burman design using `{FrF2}`.
...
It certainly is a *Lapalissade*, but to analyze data, we first need *data*. If this data is already available in R, then the analysis can be performed directly. However, in much cases, the data is stored outside the R environment, and needs to be imported.
