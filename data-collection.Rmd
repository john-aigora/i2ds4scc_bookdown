...
> Before the statistical analysis and vizualizations, the foremost proceduce step refers to a proper experimental design in a way that would assure relevant and meaningful data that are obtained with maximum efficiency. This chapter will approach from the estimation of the number of panelists, design of sensory evaluation sessions and design of experiments (DoE) to valuable execution tips and data importation.
...
For difference and similarity tests, the reader may also refer to the `{sensR}` package and its `discrimSS()` function for sensory discrimination sample size calculation.
...
```
<!-- Vanessa: As we are not calling the package before the function, maybe we should not do it here? / For some reason the code takes a very long time to run for me -->
...
<!-- Vanessa: Maybe it can be useful for the reader if we explain quickly the arguments of the functions shown--> 
...
<!-- Vanessa: I don't know why, but the warnings are showing up in the book --> 
```{r echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
library(mixexp)
library(AlgDesign)
library(tidyverse)
...
MixBiscuits <- optFederov(~ -1 +x1 +x2+ x2 +x1:x2 +x1:x3 +x2:x3 +x1:x2:x3, mdes2, nTrials=9)
...
#Builds the corresponding table to be displayed in the book
...
<!-- Vanessa: As the previous code is hidden, the user can't run the one above. --> 
```{r, echo=TRUE, eval=TRUE}
Bmixt_shuffled <- Bmixt[sample(nrow(Bmixt)), ]
```

<!-- Vanessa: Maybe table and text can be rearranged so the table 8.1 shows up after a quick explanation on what this table is about. In the way it is here, the table shows before it is called in the text.  --> 
...
<!-- Vanessa: I didn't quite understand this contour curve. What does the negative mean? I would recommend adding a conclusion on the range of each ingredient related to the optimum region, so the user understands how to interpret properly --> 
...
Fortunately, we can spare the reader some of these imagination efforts and reiterate the fundamental principles of sensory evaluation that should help avoiding major pitfalls^[For a more detailed description of these principles, we refer the reader to comprehensive sensory evaluation textbooks. See for instance (Lawless and Heymann, 2010), (Civille and Carr, 2016) or (Stone, Bleibaum and Thomas, 2022)].
...
<!-- Vanessa:Crossover reference doesn't show in the text correctly --> 
...
<!-- We discussed having all prints hidden. In the first chapters there is an explanation for user. I put eval true because I assumed you want to show the outcomes. -->
...
<!-- Vanessa: Do you want to show this code? --> 
...
<!-- Vanessa: A simple solution that may worth mentioning is using rio::import_list. You import all the sheet with
one single command --> 
```{r}
