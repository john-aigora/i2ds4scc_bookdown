...
<!-- Should we justify what nbR is, and why we define it as nbR = nbP? -->
...
<!-- Do we want to keep the last sentence? Because technically, it could be possible to check for interactions except that some effects might be confounded. Moreover, checking for interaction requires duplicating the test, but then is it better to use the additional sessions to get a complete design, or a replicated BIB? -->
...
First, let's import the *Sensory Profile.xlsx* workbook using the `readxl::read_xlsx()` file, by informing as parameter the location of the file (informed in `file_path` using the package `{here}`)^[The package `{here}` is very handy as it provides an easy way to retrieve your file's path (within your working directory) by simply giving the name of the file and folder in which they are stored.] and the `sheet` we want to read from.
