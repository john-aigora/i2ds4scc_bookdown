...
In many projects, developers work on products' formula (e.g. in the food industry, in the personal care industry). In such cases, it means that factors are ingredients and that their proportions are interdependent (the total sum of all components of a mixture must be 100%). Therefore, these factors must be treated as mixture components. Mixture designs are usually represented using ternary diagrams.
...
Suppose that we want to adjust a biscuit recipe to optimize its sensory properties, we can design an experiment in which the proportion of ingredients vary. Let's play with butter, sugar, and flour. All three combined would account for 75% of the recipe and the remaining 25% would consist of other ingredients (eggs, milk, chocolate, etc.) that we won't modify here. Besides, not any amount of these three ingredients would make sense, so we need to add constraints (ex: butter varies between 15 and 30%, sugar varies between 25 and 40%, and flour varies between 30 and 50%). Given this set of constraints, we can use `mixexp::Xvert` to find the extreme vertices of our design.

However, this design would imply 11 mixtures, which is more than needed to apply a Scheffé quadratic model. To reduce the number of mixtures and still allow fitting a quadratic model, we can thus use the `optFederov()` function from `{AlgDesign}` to select a D-optimal subset. Here, let's limit to 8 products.

```{r}
library(mixexp)
library(AlgDesign)

des <- Xvert(nfac=3, uc=c(.30, .40, .50), lc=c(.15, .25, .30), ndm = 1, plot = FALSE) %>% 
      mutate_if(is.numeric, round, digits = 3)

MixBiscuits <- optFederov(~ -1 +x1 +x2+ x2 +x1:x2 +x1:x3 +x2:x3 +x1:x2:x3, des, nTrials=8)
DesignPoints(MixBiscuits$design, axislabs = c("Butter","Sugar","Flour"), pseudo = TRUE)

```


<!-- Once the biscuits are evaluated, we can use the mixexp::MixModel() function to fit a linear model and get a correct R2 estimate. Then use mixexp::MixturePlot() for contour plot with the mod = 2 option that specifies a quadratic model -->
