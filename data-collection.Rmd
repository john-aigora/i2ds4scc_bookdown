...
Alternately, the `WilliamsDesign()` function in `{SensoMineR}` allows you to create a matrix of samples (as numbers) with numbered Judges as row names and numbered Ranks as column names. You only have to specify the number of samples to be evaluated, as in the example below for 5 samples. Suppose you want to include 20 judges in the experiment, you would then need to duplicate the initial design.

```{r}
wdes5P10J <- SensoMineR::WilliamsDesign(5)
...
One may also be tempted to use incomplete balanced block designs for hedonic tests too. However, proceeding this way is likely to induce framing bias. Indeed, each participant to the consumer tests will only see part of the product set which would affect their frame of reference if the subset of product they evaluate only covers a limited area of the sensory space. For example, suppose you are designing consumer test of chocolate chip cookies and that a majority of cookies are made with milk chocolate while few cookies are made with dark chocolate chips. If a participant evaluates a set of samples that only have the milk chocolate chips, this participant will not know about the existence of dark chocolate and give very different scores compared to what they would have if they had a full view of the product category.
To reduce the risks incured by the use of BIBD, it is possible to...
