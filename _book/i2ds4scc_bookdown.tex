...
\subsubsection{Measuring human*product interactions \{-\}}\label{measuring-humanproduct-interactions--}}
...
\NormalTok{cider }\OtherTok{\textless{}{-}} \FunctionTok{read\_xlsx}\NormalTok{(file\_path) }\SpecialCharTok{\%\textgreater{}\%} 
...
Since we have a fairly small set of text, let's use the \emph{SnowBall Stopword} list as a start, and look at the terms that are shared in both our list and this stopword list:
...
As we can see, some words such as \emph{off}, \emph{not}, \emph{no}, \emph{too}, and \emph{very} would be removed. Since we prefer to keep them, we will remove them from our list.
...
For instance, words such as \emph{like}, \emph{sample}, \emph{just}, \emph{think}, or \emph{though} for instance do not seem to bring any relevant information here. Hence, we propose to remove these (together with some others).
...
\hypertarget{raw-frequencies}{%
\subsection{Raw Frequencies}\label{raw-frequencies}}
