...
For these reasons, it is hence essential to learn to manipulate data and transition from one structure to another. After presenting many different ways to transform your data, application through simple examples will be presented\footnote{In \texttt{conditionalFormatting()}, you can specify to which \texttt{rows} and \texttt{cols} the formatting applies. In this example, \texttt{cols} takes \texttt{v+1} because the first column contains the row names.}.
...
The notion of `simple manipulations' is completely arbitrary and consists in simple data transformation that could easily be performed in other software such as Excel (although we strongly recommend performing any sorts of transformation in R, not in Excel, see\ldots)
...
If this procedure of renaming variables should be applied on many variables following a structured form (e.g.~transforming names into snake\_case, camelCase, etc.), the use of the \texttt{\{janitor\}} package comes handy thanks to its \texttt{clean\_names()} function:
...
For re-ordering columns, \texttt{relocate()} is being used. This function allows re-positioning a variable before or after another variable. By re-using the \texttt{sensory} dataset, let's position \texttt{Code} between \texttt{Product} and \texttt{Shiny}. This can be specified into two different ways:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{relocate}\NormalTok{(Code, }\AttributeTok{.after=}\NormalTok{Product)}

\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{relocate}\NormalTok{(Code, }\AttributeTok{.before=}\NormalTok{Shiny)}
\end{Highlighting}
\end{Shaded}

Last but not least (regarding columns transformation) the \texttt{select()} function (from the \texttt{\{dplyr\}} package\footnote{Many other packages include a function called \texttt{select()}, hence creating conflicts: To avoid any risks of errors, we recommend calling the \texttt{select()} function using \texttt{dplyr::select()} as it formally calls the \texttt{select()} function from \texttt{\{dplyr\}}, hence avoiding any risks of error! Of course, the same procedure applies to any other functions that may suffer the same issue.}) allows selecting a set of variables, by simply informing the variables that should be kept in the dataset. Let's limit ourselves in selecting \texttt{Judge}, \texttt{Product}, and \texttt{Shiny}:
...
Here, we consider keeping the entire set of variables except for the variable \texttt{Code}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(Judge, Product, Shiny}\SpecialCharTok{:}\NormalTok{Sticky)}
\end{Highlighting}
\end{Shaded}
...
Rather than selecting variables based on their names, we can also select them based on a certain `rule' with the \texttt{where()} function. In that case, let's consider all the variables that are numerical, which automatically removes \texttt{Judge}, \texttt{Code}, and \texttt{Product} columns:
...
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(}\AttributeTok{Panellist =}\NormalTok{ Judge, }\AttributeTok{Sample =}\NormalTok{ Product, Code, Shiny}\SpecialCharTok{:}\NormalTok{Sticky, }\SpecialCharTok{{-}}\FunctionTok{starts\_with}\NormalTok{(}\StringTok{"Qtt"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Tip: If you want to transform a variable, say by changing its type, or re-writing its content, you can use \texttt{mutate()} and assign to the new variable the same name as the original one. This will overwrite the existing column with the new one. To illustrate this, let's transform the sample codes (stored in \texttt{Code}) from upper case to lower case only. This can be done by mutating \texttt{Code} into the lowercase version of \texttt{Code} (\texttt{tolower(Code)}):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{Code =} \FunctionTok{tolower}\NormalTok{(Code))}
\end{Highlighting}
\end{Shaded}
...
Since performing mathematical computation on non-numerical columns is not possible, conditions can easily be added through variants of the \texttt{mutate()} function such as \texttt{mutate\_if()}, \texttt{mutate\_at()}, or \texttt{mutate\_all()}. However, we prefer the use of \texttt{mutate()} combined with \texttt{across()} to perform such operations.
...
In other situations, we may want to merge information present in multiple columns in one. In our \texttt{sensory} data set, we can see that the sample information is spread through two columns, one containing the code (\texttt{Code}) and one the names (\texttt{Product}). To start, let's merge these two columns so that both information is stored in one column (called here \texttt{NewName}) as \emph{code-name}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory2 }\OtherTok{\textless{}{-}}\NormalTok{ sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{unite}\NormalTok{(NewName, Code, Product, }\AttributeTok{sep=}\StringTok{"{-}"}\NormalTok{)}
\NormalTok{sensory2}
\end{Highlighting}
\end{Shaded}
...
To reverse the changes (saved here in \texttt{sensory2}) and to separate a column into different variables, the function \texttt{separate()} from the \texttt{\{tidyr\}} package is required. Similarly to \texttt{unite()}, \texttt{separate()} takes as first parameter the name of the variable to split, followed by the names for the different segments generated, and of course the separated defined by \texttt{sep}. In our example, this would be done as following:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory2 }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{separate}\NormalTok{(NewName, }\FunctionTok{c}\NormalTok{(}\StringTok{"Code"}\NormalTok{,}\StringTok{"Product"}\NormalTok{), }\AttributeTok{sep=}\StringTok{"{-}"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
To define sub-set of data, the \texttt{filter()} function is being used. This function requires providing an argument that is expressed as a test, meaning that the outcome should either be TRUE (keep the value) or FALSE (discard the value). In R, this is expressed by the double `=' sign \texttt{==}. Let's filter the data to only keep the data related to sample \texttt{29PPK\_p}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(Product }\SpecialCharTok{==} \StringTok{"29PPK\_p"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{!Product\ ==\ "29PPK\_p"} or \texttt{Product\ !=\ "29PPK\_p"} means different from, and will keep all samples except \texttt{29PPK\_p};
\item
  \texttt{\%in\%\ my\_vector} keeps any value included within the vector \texttt{my\_vector} (e.g.~\texttt{Product\ \%in\%\ c("12GP\_f","12GP16PSL\_m","12GP23P\_m")});
\item
  for multiple conditions:
\item
  \texttt{\&} (read `and') is multiplicative, meaning that all the conditions need to be true (\texttt{Product\ ==\ "12GP\_f"\ \&\ Shiny\textgreater{}40});
\item
  \texttt{\textbar{}} (read `or') is additive, meaning that only one condition needs to be true (\texttt{Product\ ==\ "12GP\_f"\ \textbar{}\ Shiny\textgreater{}40})
\end{itemize}
...
For researchers who want to split their data into logical subgroup (e.g.~splitting a panel by gender, hence performing an analysis for the males and females separately), the \texttt{filter()} function is a good alternative. This is to some extent what we did earlier when we only filtered data from sample ``29PPK\_p''. To get sub-groups of data for each sample, we could repeat the same procedure for all the other samples. However, this procedure becomes tedious as the number of samples increases.
...
This function creates a list of \emph{n} elements (\emph{n} being the number of samples here), each element being the data related to one sample.
...
TIPS: With \texttt{pivot\_longer()} and any other function that requires selecting variables, it is often easier to deselect variables that we do not want to include rather than selecting all the variables of interest. Throughout the book, both solutions will be used.
...
The complementary/opposite function to \texttt{pivot\_longer()} is called \texttt{pivot\_wider()}. This function pivots data horizontally, hence reducing the number of rows and increasing the number of columns. In this case, the two main parameters to provide is which column will provide the new column names to create (\texttt{name\_from}), and what are the corresponding values to use (\texttt{values\_from}).
...
These functions are particularly useful in consumer studies, since \texttt{pivot\_longer()} and \texttt{pivot\_wider()} allows restructuring the data for analysis such as ANOVA (\texttt{pivot\_longer()} output) and preference mapping or clustering (\texttt{pivot\_wider()} structure).
...
It can appear that the interest is not in the grand mean, say, but in mean per product, or per product and panelist in case the test has been duplicated. In such cases, the \texttt{summary()} should aggregate set of values per product, or per product x panelist respectively. Such information can be passed on through the \texttt{group\_by()}.
...
A different approach consists in combining \texttt{summarise()} to \texttt{pivot\_longer()} and \texttt{pivot\_wider()}. This process requires summarising only one column by Product and Attribute:
...
What would happen if we would omit to \texttt{summarise()} the data in between the two pivoting functions? In that case, we also remove Judge and Code which were lost in the process\ldots{}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sensory }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pivot\_longer}\NormalTok{(Shiny}\SpecialCharTok{:}\NormalTok{Melting, }\AttributeTok{names\_to=}\StringTok{"Attribute"}\NormalTok{, }\AttributeTok{values\_to=}\StringTok{"Scores"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(}\SpecialCharTok{{-}}\FunctionTok{c}\NormalTok{(Judge, Code)) }\SpecialCharTok{\%\textgreater{}\%} 
...
Similarly, tables can be combined horizontally using the corresponding function \texttt{cbind()} (\texttt{\{base\}}) and/or \texttt{bind\_cols()} (\texttt{\{dplyr\}}). In this case, it is better to ensure that the rows' order is identical before combining them to ensure no mishaps.
...
the \texttt{\{dplyr\}} package. In that case, the tables do not need to be in the same order, nor from the same size, since the function will handle it.

Depending on the merging process degree to consider between tables X and Y, there are four different \texttt{*\_join()} versions to consider:
...
Let's imagine that a consumer test has been performed: In this consumer test, each consumer evaluated \emph{P} samples, the \emph{P} evaluations (provided by each consumer) is defined along \emph{P} rows. Additionally, the consumer information (incl.~gender, age, etc.) is stored in a separate file (in this case, each consumer is only associated with one row of data). When joining these two files together \texttt{by} consumer, the consumer information is duplicated \emph{P} times to match the structure of the data provided from the consumer test.
...
The three sheets contain the following information, which will need to be combined:
...
The next step is then to merge this table to \texttt{Biscuits}. In this case, since both dataset contains the full evaluation of the cookies (each consumer evaluating each product), the joining procedure needs to be done by product and by consumer simultaneously. A quick look at the data shows two important things:
...
The names that contain the product (\texttt{Samples} and \texttt{Product}) and consumers (\texttt{Consumer} and \texttt{Judge}) information are different in both dataset. To simplify our work, we could rename these columns in one of the dataset so that it matches the other, but instead we will keep the two names and inform it within \texttt{full\_join()}. This is done by informing within the \texttt{by} parameter how the column should connect following the structure: \texttt{"name\ in\ dataset\ 1"\ =\ "name\ in\ dataset\ 2"}
...
\NormalTok{biscuits }\OtherTok{\textless{}{-}} \FunctionTok{read\_xlsx}\NormalTok{(file\_path, }\AttributeTok{sheet=}\StringTok{"Biscuits"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{Consumer =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"J"}\NormalTok{,Consumer)) }\SpecialCharTok{\%\textgreater{}\%} 
...
For the importation of datasets, we have already presented the package \texttt{\{readxl\}} amongst others. For exporting results, two complementary packages (yet again, amongst others!) in terms of ease of use and flexibility in the outcome are proposed: \texttt{\{writexl\}} and \texttt{\{openxlsx\}}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#ADD SOME CODE HERE FOR AN EXAMPLE}
\end{Highlighting}
\end{Shaded}

The exportation of tables using the \texttt{\{writexl\}} package is easy, yet too simplistic (?) as it does not allow formatting the tables (except for some minor possibilities for the header), nor does it allow exporting multiple tables within the same sheet. For more advanced exporting options, the use of \textbf{\{openxlsx\}} package is preferred as it allows more flexibility in structuring and formatting the Excel output.

With \texttt{\{openxlsx\}}, the procedure starts with creating a workbook \texttt{wb} using the \texttt{createWorkbook()} function, to which we add worksheets through the \texttt{addWorksheet()} function.
...
\item
  ...
  \tightlist
  \item
    \texttt{options()} to pre-define number formatting, border colours and style that will be applied automatically to each tables;
  \item
...
A first approach to tackle this problem could be to import each file separately, and to combine them together using the \texttt{bind\_rows()} function from the \texttt{\{dplyr\}} package. However, this solution is not optimal since 1. it is very tedious when there are many different sheets, and 2. it is not automated since the code will no longer run if (say) the number of session changes.
