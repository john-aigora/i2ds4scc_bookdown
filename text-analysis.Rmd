...
By looking at our data (`cider_og`), we can notice that for each sample evaluated, respondents are providing a set of descriptions, which can be a single word (e.g. `yeasty`) or a group of words (`like it will taste dry and acidic`). We can also note that this data set is fairly well structured since the responses are separated by a `;` or `,`.
...
As can be seen, the original comments from consumers are now split into tokens, increasing the size of the file from 168 individual comments to 947 rows of tokens.
