...
> Most of this book focus on handling data through the procedure of data cleaning, transformation, analyses, or representation. This makes sense since the aim is to present ata science in the context of sensory and consumer research. But these steps may be irrelevant if analysts or reseachers are not able to communicate their findings efficiently. In Section \@ref(auto-report), we showed you how to create your report within your analysis (and how to integrate your analysis within your report). 
Such approach is a fundamental part of good data science to us as it allows the mass-production of standardized slides, hence saving a lot of time that can be (and should be) dedicated to the final presentation and the story-telling building process. Through this chapter, let's reflect on how to deliver results in an optimal way...
...
```{r summary-table, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, tab.cap="Exemplification of the approach for each type of audience."}
library(readxl)
library(tidyverse)
library(flextable)
