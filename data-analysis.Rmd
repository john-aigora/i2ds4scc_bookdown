...
As one may expect, this chapter is mostly built around the `{tidyverse}`. So let's start with importing it (together with other useful packages).
...
file_path <- here("data", "biscuits_sensory_ P_profile.xlsx")
...
Since the data also contain two qualitative variables in `Protein` and `Fiber`, they should either be removed prior to running the analysis, or better be projected as supplementary through the `quali.sup` parameter from `PCA()`. Finally, since `POpt` is a commercial product, let's not include it in the analysis per se (it is not contributing to the construction of the dimensions). Instead we project it as supplementary (through `ind.sup`) to illustrate where it would be located on the space if it were.
...
The *biscuits_traits.xlsx* file contains descriptive (i.e. *demographic*) information regarding the consumers and their food-related personality traits (i.e. *TFEQ*, see Section \@ref(example-projects) for more information). This file has three tabs denoted as *Data*, *Variables*, and *Levels*:
...
### Personality traits: TFEQ data

In the same data set, consumers also answered some questions that reflect their relation to food (REF REF). These questions can be categorized into three groups (also known as factors): 
...
```{r, echo=TRUE, eval=TRU0E, warning=FALSE, message=FALSE}
...
Last but not least, `POpt` (which coordinates are stored in `senso_pca$ind.sup$coord`) can be projected on that space in order to see if this commercial sample could be considered as an optimized sample in terms of consumers' liking/preference.
