...
For re-ordering columns, `relocate()` is being used. This function allows re-positioning a variable before or after another variable. By re-using the `sensory` dataset, let's position all the variables starting with 'Qtt' between `Product` and `Shiny`. This can be specified into two different ways:
...
Depending on the merging process degree to consider between tables X and Y, there are four different `*_join()` versions to consider:
...
Let's imagine that a consumer test has been performed: In this consumer test, each consumer evaluated *P* samples, the *P* evaluations (provided by each consumer) is defined along *P* rows. Additionally, the consumer information (incl. gender, age, etc.) is stored in a separate file (in this case, each consumer is only associated with one row of data). When joining these two files together `by` consumer, the consumer information is duplicated *P* times to match the structure of the data provided from the consumer test.
...
The next step is then to merge this table to `Biscuits`. In this case, since both dataset contains the full evaluation of the cookies (each consumer evaluating each product), the joining procedure needs to be done by product and by consumer simultaneously. A quick look at the data shows two important things:
...
The names that contain the product (`Samples` and `Product`) and consumers (`Consumer` and `Judge`) information are different in both dataset. To simplify our work, we could rename these columns in one of the dataset so that it matches the other, but instead we will keep the two names and inform it within `full_join()`. This is done by informing within the `by` parameter how the column should connect following the structure: `"name in dataset 1" = "name in dataset 2"`
