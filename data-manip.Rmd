...
For these reasons, it is essential to learn to manipulate data and transition from one structure to another. After presenting many different ways to transform your data, application through simple examples will be presented^[Most of the examples presented in this chapter emphasize on the "how to?", not the "why?" and thus do not necessary intend to convey scientific meaning].
...
In our sensory file^[The *sensory* data is imported from the *Sensory Profile.xlsx* file, see Chapter XXX for how to import datasets], let's recode 'Judge' into 'Panellist', and 'Product' into 'Sample' (here, we apply transformations without saving the results, so without altering the original dataset):
