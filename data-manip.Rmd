...
So the data set that we are using here are *Sensory Profile.xlsx*, 

Most of the functions we use are accessible from the `{tidyverse}` package.
...
Of course, we could get sub-groups of data for each sample by repeating the same procedure for all the other samples. However, this procedure becomes tedious as the number of samples increases. Instead, we prefer to use `split()` which takes as argument the column to split from:


```{r}
split(sensory$Product)
