...
The merging procedure requires the users to provide a *key*, i.e. a (set of) variable(s) used to combine the tables. For each unique element defined by the key, a line is being created. In needed, rows of a table are being duplicated. WIthin the different `*_join()` functions, the key is informed by the `by` parameter, which may contain one or more variables, which can be have the same or different names in X and Y.
...
To illustrate, let's use the dataset ...
