...
```{r, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Scatter plot with emphasis on the display through `theme()`, `scale_continuous()`, `coord_fixed()`, `ggtitle()` and `guides()`"}
line_p
```
...
* Line charts:
  ...
  * `geom_smooth()`: add a regression line (see example Section \@ref(linechart));
...
* Bar charts: 
  * `geom_col()` and `geom_bar()`: produce bar-charts by either using the raw values, or by computing the frequencies first (see example Section \@ref(linechart));
...
knitr::include_graphics("images/multiplePlot.jpg")
```
