...
Let's imagine we want to communicate the sensory profiles of the 11 biscuits on the attributes related to appearance, a first step consists in transforming the data to create such results (see \@ref(means) for more details).
...
```{r flextablebasic, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE, tab.cap="Appearance Attributes Mean (Basic Flextable)"}
library(flextable)  
...
```{r flextablebasic2, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE, tab.cap="Appearance Attributes Mean (Basic Flextable 2)"}
...
```{r, flextabledesigned, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE, tab.cap="Appearance Attributes Mean (Improved Flextable)"}
flex_table_design %>%
...
Lastly, you can use conditional formatting if you want to highlight some specific values (e.g. values for `Thickness` that are below 20 should be colored in blue, and above 40 in red), as in the example below:
