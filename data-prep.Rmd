...
As can be seen, since `P04` contains the missing value detected, its corresponding mean is defined as `NA`. To enforce the mean to be computed, we need to inform R to remove missing values when computing the mean. This is done by adding within the `mean()` function the parameter `na.rm=TRUE`:
...
Removing rows with missing values can be done in different ways.

 - removing rows: watch out because you can remove data as well
 - removing rows in a long format means not removing anything...since the wider format will re-introduce missing values (example of incomplete design: non-visible NAs are also NAs)
 - strategies on removing the judge/product/attribute that contains missing values

#### Impute missing values

solution to impute by:
 - a fixed value
 - manually the mean by attribute/product/judge & replica
 - impute using a model (linear or more advanced)
 - EM algorithm (`{missMDA}`)
